...
The biggest difference between these educational systems is not the exam format, but **how the AI must treat and penalize student errors**. This is controlled through injections in the C3 Layer (`preprocessing_directives`), which alter the "System Prompt" of the LangGraph orchestrator before calling the LLM.
...
4. **Evaluation (LangGraph + C3):** The payload reaches the backend. LangGraph injects the behavioral modifications (`BLOCK_DIRECTIVE`), assembles the full context, and forces the LLM to return a structured schema (Structured Output).
